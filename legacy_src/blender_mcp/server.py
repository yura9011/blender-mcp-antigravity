...
# CRITICAL: Set binary mode for stdin/stdout on Windows FIRST
import sys
import os
...
# Force logging to stderr with force=True to override any previous configuration
logging.basicConfig(
    level=logging.INFO,
...
# Dummy replacements to prevent start up crash
def record_startup(*args, **kwargs): pass
def get_telemetry(*args, **kwargs): return None
def telemetry_tool(name):
    def decorator(func):
        return func
    return decorator
...
def main():
    """Run the MCP server"""
    # CRITICAL: Disable ALL logging before MCP runs to ensure clean stdio
    # Some versions of FastMCP or dependencies might log to stdout
    import logging
    ...
    # Ensure our logger also uses stderr
